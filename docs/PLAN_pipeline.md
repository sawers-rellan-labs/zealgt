...
| 6 | 2 | `ancestry_inference` | LINE_ALLELE_COUNTS → RTIGER (rigidity 500; RTIGER learns its parameters from the data, no design prior — §4 #10) | donor × chr | ancestry segments per line |
...
| 10 | **nilHMM ignores the design prior for RTIGER** (zealbc1 `docs/notebooks/06_fullcov_union_pilot_zx0540_zx0570.qmd` §3.3): `call_states()` calls `.rtiger_states(data, rigidity, …)` without `design`; the "BC2S3" and "BC2S2" RTIGER runs were byte-identical, so "RTIGER with design BC2S3" never applied | stage 6 | the stage is described as RTIGER without a design prior; a BC2S2-vs-BC2S3 test for the pooled lines needs a caller that uses `design` (nilHMM `binhmm`, `fsfhap`, `pedigree`) |
...
| 12 | **PHG and RTIGER fall short of the single-locus expectation** (zealbc1 `docs/notebooks/06_fullcov_union_pilot_zx0540_zx0570.qmd` §2.4): both place introgressions in the same lines and positions, but PHG splits RTIGER TEO segments into TEO/HET/B73 and calls recurring short HET spikes at the same ranges in lines RTIGER calls all B73; TEO allele fraction 7.0–8.0 % (PHG) and 8.7–10.8 % (RTIGER) vs 12.5 % expected; within one family no dilution is expected | stage 7 vs 6; calibration gate | **open:** test PHG over-calling B73 (reference bias) vs founder holes — per range, founder missing sites where PHG disagrees with RTIGER inside introgressions and at the recurring spikes vs where they agree. Report PHG shares over called ranges together with the no-call share (24–30 %), RTIGER over bp |
| 13 | **One low-coverage line stops the whole RTIGER run**: RTIGER needs 2 × rigidity covered markers per line (1,000 at rigidity 500); PN6_SID484 had 253 and was dropped by hand | stage 2b COVERAGE_QC → stage 6 | the coverage floor per caller is applied before ancestry inference; a line below it is left out of RTIGER (and reported), not allowed to abort the run. The two failed development libraries are excluded in `meta/dev_import.csv` |
...
§4 #2, #3, #5–#7, #12, #13; storage rules of §5; which existing CRAMs and tables are reused vs regenerated after MARKDUP.
